...
a=f=1);c=1!==a||w?t+"/p"+a+k:t;c!==window.location.href&&Q&&history.replaceState(null,null,c);h.is(":focus")||h.val(a).data("page",a.toString());R.text(f);I.go(f/r*99.9)}}function J(a){a=b('<div class="ScrollProgress"/>').insertBefore(a);b.fn.spin?a.spin({lines:9,radius:6,length:6,width:3}):a.html('<span class="Progress"/>')}function S(a){d=!0;J(a);b.get(t+"/p"+f+"?DeliveryType=VIEW&InnerList=1",function(c){b(c).appendTo(l).first().addClass("InfScrollHighlight").find(":not(tr)").first().prepend('<a id="Page_'+
f+'"/>');f+=1;f>n&&(m.find(".CommentForm").show(),b(a).remove());g.trigger("CommentPagingComplete")}).always(function(){d=!1;b(".ScrollProgress").remove();A()})}function T(a){d=!0;J(a);b.get(t+"/p"+e,function(c){var d=g.height(),f=v.scrollTop();b("#Content ul.DataList.Comments, main.page-content ul.DataList.Comments, #Content ul.DataList.Discussions, main.page-content ul.DataList.Discussions, #Content table.DataTable.DiscussionsTable tbody, main.page-content table.DataTable.DiscussionsTable tbody",
c).children().prependTo(l).last().addClass("InfScrollHighlight").end().first().find(":not(tr)").first().prepend('<a id="Page_'+e+'"/>');b("div.ItemDiscussion",c).appendTo(y);--e;0===e&&(b(a).remove(),x(!0));z.remove();g.scrollTop(f+g.height()-d);q=l.offset();g.trigger("CommentPagingComplete")}).always(function(){d=!1;b(".ScrollProgress").remove();A()})}function p(){var a,b;a=Date.now();a>K+100&&(K=a,A());d||1>e&&n<f||(a=document.getElementById("PagerAfter"),b=document.getElementById("PagerBefore"),
u(a,L)&&f<=n?S(a):u(b,L)&&0<e&&T(b))}function M(a){-1===a?a=q.top+l.height():2>a?(a=w?b("#Item_0").offset().top:0,x(!0)):a=b("#Page_"+a).offset().top;return b("html, body").animate({scrollTop:a})}function B(a){if(!d){var c=a;2>a&&(a=-1===a?n:1);e<a&&a<f?M(c).promise().always(function(){p()}):(d=!0,m.css("visibility","hidden"),C.show(),b.get(t+"/p"+a,function(e){m.replaceWith(b("#Content, main.page-content",e));G(a);M(c).promise().always(function(){d=!1;p()})}).always(function(){m.css("visibility",
"visible");C.hide()}))}}var v=b(window),g=b(document),Q=!(!window.history||!window.history.replaceState),d,e=c("Page",1)-1,n=c("TotalPages",1),f=e+2,r=c("CountItems"),H=c("PerPage"),w=c("InDiscussion",!1),O=c("HideHead",!0),L=c("Treshold",300),t=c("Url",!1),z=b("<div/>"),K=0,D=null,I,R=b("#InfScrollNav .NavIndex"),P=b("#Panel, aside.page-sidebar"),h=b("#InfScrollJT"),N=b("#Frame"),C=b("#PageProgress"),F,l,q,y,m;!N.length&&b.fn.spin&&(N=b("body > *"),C.spin({lines:11,radius:10,length:10,width:4}));
I=new Nanobar({bg:1!==r?c("ProgressBg"):"transparent",id:"ProgressBar",target:0!==c("NavStyle",0)?b("#InfScrollNav .PageCount")[0]:null});d=!0;setTimeout(function(){location.hash&&(location.hash=location.hash.toString());d=!1;p()},500);G(c("Page",1));v.scroll(p);v.scroll(function(){null!==D&&clearTimeout(D);D=setTimeout(p,100)});b("#InfScrollNav .JTT").click(function(a){a.preventDefault();B(0)});b("#InfScrollNav .JTB").click(function(a){a.preventDefault();B(-1)});g.click(function(a){var d=b("#InfScrollNav");
...
return!1}).select()}).focusout(function(){h.data("page")!==h.val()&&c("Mobile",!1)&&h.data("page",h.val()).parent().submit()});b("#InfScrollNav form.JumpTo").submit(function(){var a=parseInt(h.val(),10);0<a&&a<=n&&B(a);return!1});g.on("CommentAdded",function(){r+=1;b("#InfScrollNav .PageCount span.small").text(r);p()})});
